*> \brief \b CGECXX computes a CX factorization of a real M-by-N matrix A using a truncated (rank k) Householder QR factorization with column pivoting.
*
*  =========== DOCUMENTATION ===========
*
...
*
*> \htmlonly
*> Download CGECXX + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/cgecxx.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/cgecxx.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/cgecxx.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*       SUBROUTINE CGECXX( FACT, USESD, M, N,
...
*> \verbatim
*>
*> CGECXX computes a CX factorization of a real M-by-N matrix A using
*> a truncated rank-K Householder QR factorization with a column
*> pivoting algorithm, which is implemented in the CGEQP3RK routine.
*>
...
*>                preselected columns using the routine CGEQRF.
*>
...
*>                column pivoting using the routine CGEQP3RK on
...
*> \param[out] TAU
*> \verbatim
*>          TAU is COMPLEX array, dimension (min(M_sub,N_sub))
...
*>          also on:
*>           1a) column preselection stage using CGEQRF:
*>               the optimal block size NB, the crossover point NX
*>               returned by ILAENV for the routine CGEQRF
*>               in comparison to N_sel. (For N_sel <= NX
*>               or N_sel <= NB, unblocked code is used in CGEQRF.)
*>           1b) column preselection stage using CUNMQR:
*>               the optimal block size NB returned by ILAENV for
*>               the routine CUNMQR in comparison to N_sel. (For
*>               N_sel <= NB, unblocked code is used in CUNMQR.)
*>            2) column selection stage via criteria using CGEQRP3RK:
*>               the optimal block size NB, the crossover point NX
*>               returned by ILAENV for the routine CGEQRP3RK
*>               in comparison to min(M,N_sel). (For
*>               min(M_sub, N_free, KMAXFREE) <= NX
*>               or min(M_sub, N_free, KMAXFREE) <= NB, unblocked code
*>               is used in CGEQRP3RK.)
*>           3a) computation of the factor X using CGEQRFin CGELS:
*>               the optimal block size NB, the crossover point NX
*>               returned by ILAENV for the routine CGEQRF
*>               in comparison to K. (For K <= NX or K <= NB,
*>               unblocked code is used in CGEQRFinside CGELS.)
*>           3b) computation of the factor X using CUNMQR in CGELS:
*>               the optimal block size NB returned by ILAENV for
*>               the routine CUNMQR in comparison to N. (For
*>               N <= NB, unblocked code is used in CUNMQR
*>               inside CGELS.)
*> \endverbatim
*>
*> \param[out] RWORK
...
*     .. External Subroutines ..
      EXTERNAL           CCOPY, CGELS, CGEQP3RK, CGEQRF, CLACPY,
     $                   CUNMQR, CSWAP, XERBLA
*     ..
*     .. External Functions ..
      LOGICAL            SISNAN, LSAME
      INTEGER            ICAMAX, ILAENV
      REAL               SLAMCH, CLANGE, SCNRM2
      EXTERNAL           SISNAN, SLAMCH, CLANGE, SCNRM2, ICAMAX,
     $                   ILAENV, LSAME
*     ..
*     .. Intrinsic Functions ..
...
*
*              Query for optimal workspace size for CGEQRF.
*
               CALL CGEQRF( MSUB, NSEL, A, LDA, TAU, WORK,
     $                         -1, IINFO )
...
*
*                 Query for optimal workspace size for CUNMQR.
*
                  CALL CUNMQR( 'L', 'C', MSUB, NFREE,
...
*              computation.
*              LWKMIN = MAX(1, NFREE-1) for the call of CGEQP3RK.
*
               LWKMIN = MAX( LWKMIN, NFREE - 1 )
*
*              Query for optimal workspace size for CGEQP3RK.
*
               CALL CGEQP3RK( MFREE, NFREE, 0, NFREE,
     $                        MINUSONE, MINUSONE,
...
*              LRWKMIN = MAX(1, 2*NFREE) for the call of CGEQP3RK.
*
               LRWKMIN = MAX( LRWKMIN, 2*NFREE )
*
*              (Int_wk_part_2) Integer minimum workspace computation.
*              LIWKMIN =  NFREE-1 for the call of CGEQP3RK.
*
...
*                 NFREE is for CGEQP3RK and NFREE-1 for JPIV adjustment.
*
...
*                     = max( 1, MINMN + N ) for the call of CGELS.
*
               LWKMIN = MAX( LWKMIN, MINMN + N )
*
*              Query for optimal workspace size for CGELS.
*
               KMAXLS = MINMN
*
               CALL CGELS( 'N', M, KMAXLS, N, QRC, LDQRC, X, LDX,
     $                     WORK, -1, IINFO )
               LWKOPT = MAX( LWKOPT, INT( WORK(1) ) )
*
            END IF
...
*
                  CALL CSWAP( N, A( I, 1 ), LDA, A( MSUB, 1 ), LDA )
*
...
*
                  CALL CSWAP( M, A( 1, J ), 1, A( 1, NSEL ), 1 )
                  JPIV( J ) = JPIV( NSEL )
                  JPIV( NSEL ) = J
...
*
                  CALL CSWAP( M, A( 1, J ), 1, A( 1, JDESEL ), 1 )
                  ITEMP = JPIV( J )
                  JPIV( J ) = JPIV( JDESEL )
                  JPIV( JDESEL ) = ITEMP
...
*              A_sub_resid(NSEL) = A(NSEL+1:MSUB, NSEL+1:NSUB)
*
         CALL CGEQRF( MSUB, NSEL, A, LDA, TAU, WORK, LWORK, IINFO )
*
...
*
            CALL CUNMQR( 'L', 'C', MSUB, NFREE, NSEL,
...
*     ==================================================================
*
*        Disable RELTOLFREE when calling CGEQP3RK for free columns
*        factorization, since CGEQP3RK expects RELTOLFREE with respect
...
*
         ...
         END IF
*
         CALL CGEQP3RK( MFREE, NFREE, 0, KMAXFREE,
...
*
      IF( MIN( MRESID, NRESID ).NE.0 ) THEN
         FNRMK = CLANGE( 'F',  MRESID, NRESID, A( K+1, K+1 ),
     $                   LDA, WORK )
      ELSE
         FNRMK = ZERO
      END IF
...
*
         DO J = 1, K, 1
            CALL CCOPY( M, X( 1, JPIV( J ) ), 1, C( 1, J ), 1 )
         END DO
*
      ELSE
*
...
*        of the routine.
*
         CALL CLACPY( 'F', M, K, C, LDC, QRC, LDQRC )
*
         CALL CGELS( 'N', M, K, N, QRC, LDQRC, X, LDX,
     $               WORK, LWORK, IINFO )
         INFO = IINFO
*
      END IF
