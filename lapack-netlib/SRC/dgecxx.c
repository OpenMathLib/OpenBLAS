...
/* Subroutine */ void dgecxx_(char *fact, char *usesd, integer *m, integer *n, 
	integer *desel_rows__, integer *sel_desel_cols__, integer *kmaxfree, 
	doublereal *abstol, doublereal *reltol, doublereal *a, integer *lda, 
	integer *k, doublereal *maxc2nrmk, doublereal *relmaxc2nrmk, 
	doublereal *fnrmk, integer *ipiv, integer *jpiv, doublereal *tau, 
	doublereal *c__, integer *ldc, doublereal *qrc, integer *ldqrc, 
	doublereal *x, integer *ldx, doublereal *work, integer *lwork, 
	integer *iwork, integer *liwork, integer *info)
{
    ...
    /* Local variables */
    doublereal maxc2nrm, relmaxc2nrmkfree;
    ...
    doublereal eps;
    integer nsel, msub, nsub;
    extern doublereal dnrm2_(integer *, doublereal *, integer *);
    integer kfree, mfree, nfree;
    extern /* Subroutine */ void dgels_(char *, integer *, integer *, integer *
    ...
    logical usetol;
    integer lwkopt;
    logical lquery, use_desel_rows__;
    integer liwkmin;
    logical returnc;
    integer liwkopt;
    logical returnx;
    extern /* Subroutine */ void dgeqp3rk_(integer *, integer *, integer *, 
...
/*              Query for optimal workspace size for DGEQRF. */
		...
		dgeqrf_(&msub, &nsel, &a[a_offset], lda, &tau[1], &work[1], &
			c_n1, &iinfo);
/* Computing MAX */
...
/*                 Query for optimal workspace size for DORMQR. */
		    ...
		    dormqr_("L", "T", &msub, &nfree, &nsel, &a[a_offset], lda,
...
/* Computing MAX */
		i__1 = lwkmin, i__2 = nfree * 3 - 1;
		lwkmin = f2cmax(i__1,i__2);

/*              Query for optimal workspace size for DGEQP3RK. */

		dgeqp3rk_(&mfree, &nfree, &c__0, &nfree, &c_b15, &c_b15, &a[
...
/*                     = f2cmax( 1, MINMN + N ) for the call of DGELS. */
...
/*              Query for optimal workspace size for DGELS. */
		...
		dgels_("N", m, &kmaxls, n, &qrc[qrc_offset], ldqrc, &x[
...
/*     A into the array X. */
    ...
    if (returnx) {
	dlacpy_("F", m, n, &a[a_offset], lda, &x[x_offset], ldx);
    }
...
/*     the array X. */
    ...
    if (returnc && ! returnx) {
	dlacpy_("F", m, n, &a[a_offset], lda, &c__[c_offset], ldc);
    }
...
/*                 Here, we swap A(I,1:N) into A(MSUB,1:N). */
		    ...
		    dswap_(n, &a[i__ + a_dim1], lda, &a[msub + a_dim1], lda);
...
/*                 Here, we swap the column A(1:M,J) into A(1:M,NSEL) */
		    ...
		    dswap_(m, &a[j * a_dim1 + 1], &c__1, &a[nsel * a_dim1 + 1]
			    , &c__1);
		    jpiv[j] = jpiv[nsel];
		    jpiv[nsel] = j;
...
/*                 Here, we swap the column A(1:M,J) into A(1:M,JDESEL) */
		    ...
		    dswap_(m, &a[j * a_dim1 + 1], &c__1, &a[jdesel * a_dim1 + 
			    1], &c__1);
		    itemp = jpiv[j];
		    jpiv[j] = jpiv[jdesel];
		    jpiv[jdesel] = itemp;
...
/*     A_sub = A(1:MSUB, 1:NSUB) and store them in WORK(1:NSUB). */
    ...
    i__1 = nsub;
    for (j = 1; j <= i__1; ++j) {
	work[j] = dnrm2_(&msub, &a[j * a_dim1 + 1], &c__1);
    }
...
/*              A_sub_resid(NSEL) = A(NSEL+1:MSUB, NSEL+1:NSUB) */
	...
	dgeqrf_(&msub, &nsel, &a[a_offset], lda, &tau[1], &work[1], lwork, &
		iinfo);
...
/*           This is only for case (c-2) ('L' = Left, 'T' = Transpose) */
	    ...
	    dormqr_("L", "T", &msub, &nfree, &nsel, &a[a_offset], lda, &tau[1]
...
/*        Save JPIV(NSEL+1:NSUB) into WORK(NFREE+1:2*NFREE-1) */
	...
	dgeqp3rk_(&mfree, &nfree, &c__0, kmaxfree, &abstolfree, &reltolfree, &
		a[nsel + 1 + (nsel + 1) * a_dim1], lda, &kfree, &
		maxc2nrmkfree, &relmaxc2nrmkfree, &jpiv[nsel + 1], &tau[nsel 
		+ 1], &work[1], lwork, &iwork[1], &iinfo);
...
/*     respectively in  A_free_resid = A(K+1:MSUB,K+1:NSUB). */
	...
	*fnrmk = dlange_("F", &mresid, &nresid, &a[*k + 1 + (*k + 1) * a_dim1]
		, lda, &work[1]);
    } else {
	*fnrmk = 0.;
    }
...
/*        doing column permutation in place, as it is the ELSE case. */
	    ...
	    i__1 = *k;
	    for (j = 1; j <= i__1; ++j) {
		dcopy_(m, &x[jpiv[j] * x_dim1 + 1], &c__1, &c__[j * c_dim1 + 
			1], &c__1);
	    }
...
/*              index J. */
		    ...
		    dswap_(m, &c__[j * c_dim1 + 1], &c__1, &c__[jp * c_dim1 + 
			    1], &c__1);
...
/*        of the routine. */
	...
	dlacpy_("F", m, k, &c__[c_offset], ldc, &qrc[qrc_offset], ldqrc);

	dgels_("N", m, k, n, &qrc[qrc_offset], ldqrc, &x[x_offset], ldx, &
		work[1], lwork, &iinfo);
	*info = iinfo;
