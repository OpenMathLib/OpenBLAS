...
/* Subroutine */ int sgecxx_(char *fact, char *usesd, integer *m, integer *n, 
	...
	jpiv, real *tau, real *c__, integer *ldc, real *qrc, integer *ldqrc, 
	real *x, integer *ldx, real *work, integer *lwork, integer *iwork, 
...
/*              Query for optimal workspace size for SGEQRF. */
		...
		sgeqrf_(&msub, &nsel, &a[a_offset], lda, &tau[1], &work[1], &
			c_n1, &iinfo);
/* Computing MAX */
...
/*                 Query for optimal workspace size for SORMQR. */
		    ...
		    sormqr_("L", "T", &msub, &nfree, &nsel, &a[a_offset], lda,
...
/*              Query for optimal workspace size for SGEQP3RK. */
		...
		sgeqp3rk_(&mfree, &nfree, &c__0, &nfree, &c_b15, &c_b15, &a[
...
/*                     = f2cmax( 1, MINMN + N ) for the call of SGELS. */
...
/*              Query for optimal workspace size for SGELS. */
		...
		sgels_("N", m, &kmaxls, n, &qrc[qrc_offset], ldqrc, &x[
...
/*     A into the array X. */
    ...
    if (returnx) {
	slacpy_("F", m, n, &a[a_offset], lda, &x[x_offset], ldx);
    }
...
/*     the array X. */
    ...
    if (returnc && ! returnx) {
	slacpy_("F", m, n, &a[a_offset], lda, &c__[c_offset], ldc);
    }
...
/*                 Here, we swap A(I,1:N) into A(MSUB,1:N). */
		    ...
		    sswap_(n, &a[i__ + a_dim1], lda, &a[msub + a_dim1], lda);
...
/*                 Here, we swap the column A(1:M,J) into A(1:M,NSEL) */
		    ...
		    sswap_(m, &a[j * a_dim1 + 1], &c__1, &a[nsel * a_dim1 + 1]
			    , &c__1);
		    jpiv[j] = jpiv[nsel];
		    jpiv[nsel] = j;
...
/*                 Here, we swap the column A(1:M,J) into A(1:M,JDESEL) */
		    ...
		    sswap_(m, &a[j * a_dim1 + 1], &c__1, &a[jdesel * a_dim1 + 
			    1], &c__1);
		    itemp = jpiv[j];
		    jpiv[j] = jpiv[jdesel];
		    jpiv[jdesel] = itemp;
...
/*     A_sub = A(1:MSUB, 1:NSUB) and store them in WORK(1:NSUB). */
    ...
    i__1 = nsub;
    for (j = 1; j <= i__1; ++j) {
	work[j] = snrm2_(&msub, &a[j * a_dim1 + 1], &c__1);
    }
...
/*              A_sub_resid(NSEL) = A(NSEL+1:MSUB, NSEL+1:NSUB) */
	...
	sgeqrf_(&msub, &nsel, &a[a_offset], lda, &tau[1], &work[1], lwork, &
		iinfo);
...
/*           This is only for case (c-2) ('L' = Left, 'T' = Transpose) */
	    ...
	    sormqr_("L", "T", &msub, &nfree, &nsel, &a[a_offset], lda, &tau[1]
...
/*        Save JPIV(NSEL+1:NSUB) into WORK(NFREE+1:2*NFREE-1) */
	...
	sgeqp3rk_(&mfree, &nfree, &c__0, kmaxfree, &abstolfree, &reltolfree, &
...
/*     respectively in  A_free_resid = A(K+1:MSUB,K+1:NSUB). */
	...
	*fnrmk = slange_("F", &mresid, &nresid, &a[*k + 1 + (*k + 1) * a_dim1]
		, lda, &work[1]);
    } else {
	*fnrmk = 0.f;
    }
...
/*        doing column permutation in place, as it is the ELSE case. */
	    ...
	    i__1 = *k;
	    for (j = 1; j <= i__1; ++j) {
		scopy_(m, &x[jpiv[j] * x_dim1 + 1], &c__1, &c__[j * c_dim1 + 
			1], &c__1);
	    }
...
/*              index J. */
		    ...
		    sswap_(m, &c__[j * c_dim1 + 1], &c__1, &c__[jp * c_dim1 + 
			    1], &c__1);
...
/*        of the routine. */
	...
	slacpy_("F", m, k, &c__[c_offset], ldc, &qrc[qrc_offset], ldqrc);

	sgels_("N", m, k, n, &qrc[qrc_offset], ldqrc, &x[x_offset], ldx, &
		work[1], lwork, &iinfo);
	*info = iinfo;
