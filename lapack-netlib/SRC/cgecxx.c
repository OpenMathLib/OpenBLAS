...
/* Subroutine */ void cgecxx_(char *fact, char *usesd, integer *m, integer *n, 
	integer *desel_rows__, integer *sel_desel_cols__, integer *kmaxfree, 
	real *abstol, real *reltol, complex *a, integer *lda, integer *k, 
	real *maxc2nrmk, real *relmaxc2nrmk, real *fnrmk, integer *ipiv, 
	integer *jpiv, complex *tau, complex *c__, integer *ldc, complex *qrc,
	 integer *ldqrc, complex *x, integer *ldx, complex *work, integer *
	lwork, real *rwork, integer *lrwork, integer *iwork, integer *liwork, 
	integer *info)
{
    ...
    /* Local variables */
    real maxc2nrm, relmaxc2nrmkfree;
    ...
    real eps;
    integer nsel, msub, nsub, kfree;
    extern /* Subroutine */ void cgels_(char *, integer *, integer *, integer *
    ...
    extern /* Subroutine */ void ccopy_(integer *, complex *, integer *, 
	    ...
	    complex *, integer *);
    extern real scnrm2_(integer *, complex *, integer *), clange_(char *, 
    ...
    logical returnx;
    extern /* Subroutine */ void cgeqp3rk_(integer *, integer *, integer *, 
	    integer *, real *, real *, complex *, integer *, integer *, real *
	    , real *, integer *, complex *, complex *, integer *, real *, 
	    integer *, integer *);
...
/*              Query for optimal workspace size for CGEQRF. */
		...
		cgeqrf_(&msub, &nsel, &a[a_offset], lda, &tau[1], &work[1], &
			c_n1, &iinfo);
/* Computing MAX */
...
/*                 Query for optimal workspace size for CUNMQR. */
		    ...
		    cunmqr_("L", "C", &msub, &nfree, &nsel, &a[a_offset], lda,
...
/*              Query for optimal workspace size for CGEQP3RK. */
		...
		cgeqp3rk_(&mfree, &nfree, &c__0, &nfree, &c_b15, &c_b15, &a[
...
/*                     = f2cmax( 1, MINMN + N ) for the call of CGELS. */
...
/*              Query for optimal workspace size for CGELS. */
		...
		cgels_("N", m, &kmaxls, n, &qrc[qrc_offset], ldqrc, &x[
...
/*     A into the array X. */
    ...
    if (returnx) {
	clacpy_("F", m, n, &a[a_offset], lda, &x[x_offset], ldx);
    }
...
/*     the array X. */
    ...
    if (returnc && ! returnx) {
	clacpy_("F", m, n, &a[a_offset], lda, &c__[c_offset], ldc);
    }
...
/*                 Here, we swap A(I,1:N) into A(MSUB,1:N). */
		    ...
		    cswap_(n, &a[i__ + a_dim1], lda, &a[msub + a_dim1], lda);
...
/*                 Here, we swap the column A(1:M,J) into A(1:M,NSEL) */
		    ...
		    cswap_(m, &a[j * a_dim1 + 1], &c__1, &a[nsel * a_dim1 + 1]
			    , &c__1);
		    jpiv[j] = jpiv[nsel];
		    jpiv[nsel] = j;
...
/*                 Here, we swap the column A(1:M,J) into A(1:M,JDESEL) */
		    ...
		    cswap_(m, &a[j * a_dim1 + 1], &c__1, &a[jdesel * a_dim1 + 
			    1], &c__1);
		    itemp = jpiv[j];
		    jpiv[j] = jpiv[jdesel];
		    jpiv[jdesel] = itemp;
...
/*     A_sub = A(1:MSUB, 1:NSUB) and store them in WORK(1:NSUB). */
    ...
    i__1 = nsub;
    for (j = 1; j <= i__1; ++j) {
	rwork[j] = scnrm2_(&msub, &a[j * a_dim1 + 1], &c__1);
    }
...
/*              A_sub_resid(NSEL) = A(NSEL+1:MSUB, NSEL+1:NSUB) */
	...
	cgeqrf_(&msub, &nsel, &a[a_offset], lda, &tau[1], &work[1], lwork, &
		iinfo);
...
/*           This is only for case (c-2) ('L' = Left, 'T' = Transpose) */
	    ...
	    cunmqr_("L", "C", &msub, &nfree, &nsel, &a[a_offset], lda, &tau[1]
...
/*        that the criterion is disabled. */
	...
	if (usetol) {
/* Computing MAX */
	    r__1 = *abstol, r__2 = *reltol * maxc2nrm;
	    abstolfree = f2cmax(r__1,r__2);
	} else {
	    abstolfree = -1.f;
	}
	reltolfree = -1.f;
...
/*        Save JPIV(NSEL+1:NSUB) into WORK(NFREE+1:2*NFREE-1) */
	...
	cgeqp3rk_(&mfree, &nfree, &c__0, kmaxfree, &abstolfree, &reltolfree, &
...
/*     respectively in  A_free_resid = A(K+1:MSUB,K+1:NSUB). */
	...
	*fnrmk = clange_("F", &mresid, &nresid, &a[*k + 1 + (*k + 1) * a_dim1]
		, lda, &work[1]);
    } else {
	*fnrmk = 0.f;
    }
...
/*        doing column permutation in place, as it is the ELSE case. */
	    ...
	    i__1 = *k;
	    for (j = 1; j <= i__1; ++j) {
		ccopy_(m, &x[jpiv[j] * x_dim1 + 1], &c__1, &c__[j * c_dim1 + 
			1], &c__1);
	    }
...
/*              index J. */
		    ...
		    cswap_(m, &c__[j * c_dim1 + 1], &c__1, &c__[jp * c_dim1 + 
			    1], &c__1);
...
/*        of the routine. */
	...
	clacpy_("F", m, k, &c__[c_offset], ldc, &qrc[qrc_offset], ldqrc);

	cgels_("N", m, k, n, &qrc[qrc_offset], ldqrc, &x[x_offset], ldx, &
		work[1], lwork, &iinfo);
	*info = iinfo;
